...
def test_rollout_verifier_accepts_valid_shadow_evidence(tmp_path):
    ...
    result = verify_ml_rl_rollout(
        str(tmp_path), str(report_path), str(shadow_path)
    )
...
def test_rollout_verifier_rejects_manifest_checksum_and_feature_hash(tmp_path):
    ...
    (tmp_path / "model.pkl.metadata.json").write_text(
    ...
    _write_report(
        report_path,
        _report(metadata={"feature_contract_hash": "features-v1", "model_paths": ["model.pkl"]}),
    )
    _shadow(shadow_path)
