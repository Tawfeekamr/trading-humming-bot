...
class TestPositionGuardNegativeOrders:
    ...
    def test_rejects_negative_order_usdt(self):
        ...
        pg = PositionGuard(max_base_exposure_pct=80, min_usdt_reserve=50,
                          total_capital=200)
        result = pg.can_place_order(
            current_base=0.5, base_price=200,
...
class TestPositionGuard:
    ...
    def test_blocks_order_exceeding_exposure(self):
        ...
        assert not pg.can_place_order(
            current_base=0.8, base_price=200,
    ...
    def test_blocks_order_below_usdt_reserve(self):
        pg = PositionGuard(max_base_exposure_pct=80, min_usdt_reserve=50,
                           total_capital=200)
        assert not pg.can_place_order(
            current_base=0.25, base_price=200,
            current_usdt=60, order_usdt=20
        )

    def test_base_exposure_pct_calculation(self):
        pg = PositionGuard(max_base_exposure_pct=80, min_usdt_reserve=50,
                           total_capital=200)
        pct = pg.base_exposure_pct(current_base=0.5, base_price=200)
        assert pct == 50.0
