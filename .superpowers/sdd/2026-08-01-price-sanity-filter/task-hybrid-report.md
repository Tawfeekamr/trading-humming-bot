...
- Persisted positions with an empty `tp_levels` ladder are backfilled on load. New state persists the original per-unit risk so targets remain correct after breakeven promotion. Legacy states use their original stop distance, then the legacy TP1 distance, then a non-zero trail/fallback estimate when that history is unavailable.
