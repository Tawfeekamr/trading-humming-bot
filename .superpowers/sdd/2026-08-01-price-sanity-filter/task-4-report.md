...
- Engine tracks per-symbol generations and the latest pending suspect book. A stale result re-observes the latest candidate; it is accepted, hard-rejected, or starts a fresh verification without clearing a newer pending candidate. A stale denied/unavailable result requeues a held latest candidate using the original filter candidate for correct resolution.
...
- Added focused engine tests for suspect entry veto/exit bypass, confirmed-book reprocessing, stale latest-book reprocessing, denied stale-result replacement verification, and API entry veto.

## Focused verification

- `cargo test --lib engine::tests --no-fail-fast` (14 passed)
